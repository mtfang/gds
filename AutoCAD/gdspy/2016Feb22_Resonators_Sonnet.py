...
for i in range(0,5):
    ...
    cpw.openGap(openDis)
    cpw.end()
    # primitives.append(gdspy.PolygonSet(cpw.path))
    # if i == 0:
    #     cpwTemp = cpw.makePolySet(cpw)
    # else:
    #     cpwPoly = cpw.makePolySet(cpw)
# addition = lambda p1, p2: p2 or p1
# device.add(gdspy.boolean(primitives, addition,
#                             layer=8))
print(device.get_polygons())
# device.add(cpwTemp)
# join = gdspy.boolean([cpwPoly, cpwTemp],
#     lambda one, two: one or two)
# device.add(join)
# for poly in get_polygons(by_spec=False, depth=None)
# cpwfeed = cpwfeed.makePolySet(cpwfeed)
# sonnet = gdslib.mask(cpwfeed, addThis = gdspy.Rectangle((0, 0), (10000 , 5000), 0), layer = 8, datatype = 1)
# device.add(sonnet)
# DEVICE LABEL
